...
if((isset($data)) and (stripos($data,"dan|") !== false)){
$dan = explode("|",$data)[1];
mysqli_query($conn, "UPDATE `azolar` SET `dan`='$dan' WHERE `uid`='$fid'");
$info =  mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `azolar` WHERE `uid`='$fid'"));
$keyb = [];
if($info['dan'] == 'uz'){
	array_push($keyb,[["text"=>"✅ 🇺🇿 O'zbek","callback_data"=>"dan|uz"],["text"=>"❌ 🇺🇿 O'zbek","callback_data"=>"x"]]);
	array_push($keyb,[["text"=>"🇷🇺 Russia","callback_data"=>"dan|ru"],["text"=>"🇷🇺 Russia","callback_data"=>"ga|ru"]]);
    array_push($keyb,[["text"=>"🏴󠁧󠁢󠁥󠁮󠁧󠁿 English","callback_data"=>"dan|en"],["text"=>"🏴󠁧󠁢󠁥󠁮󠁧󠁿 English","callback_data"=>"ga|en"]]);
}elseif($info['dan'] == 'ru'){
	array_push($keyb,[["text"=>"🇺🇿 O'zbek","callback_data"=>"dan|uz"],["text"=>"🇺🇿 O'zbek","callback_data"=>"ga|uz"]]);
	array_push($keyb,[["text"=>"✅ 🇷🇺 Russia","callback_data"=>"dan|ru"],["text"=>"❌ 🇷🇺 Russia","callback_data"=>"x"]]);
    array_push($keyb,[["text"=>"🏴󠁧󠁢󠁥󠁮󠁧󠁿 English","callback_data"=>"dan|en"],["text"=>"🏴󠁧󠁢󠁥󠁮󠁧󠁿 English","callback_data"=>"ga|en"]]);
}else{
	array_push($keyb,[["text"=>"🇺🇿 O'zbek","callback_data"=>"dan|uz"],["text"=>"🇺🇿 O'zbek","callback_data"=>"ga|uz"]]);
	array_push($keyb,[["text"=>"🇷🇺 Russia","callback_data"=>"dan|ru"],["text"=>"🇷🇺 Russia","callback_data"=>"ga|ru"]]);
    array_push($keyb,[["text"=>"✅ 🏴󠁧󠁢󠁥󠁮󠁧󠁿 English","callback_data"=>"dan|en"],["text"=>"❌ 🏴󠁧󠁢󠁥󠁮󠁧󠁿 English","callback_data"=>"x"]]);
}
$bot->editMessageText([
"chat_id"=>$fid,
"message_id"=>$mid,
"text"=>"<b>👋 Assalomu aleykum!
🤖 Tarjimon botga hech kelibsiz!

⚙️ Tarjima tilini quyidagi tugmalarni bosish yoki /setlang botini botga yuborish orqali sozlab olishingiz mumkin.

🏳️ Qaysi tildan  | 🏳️ Qaysi tilga
          Tarjima qilmoqchisiz ?
                
✍️ Sozlashni yakunlagan bo'lsangiz matn yuborishingiz mumkin!</b>",
"parse_mode"=>"html",
...
if((isset($data)) and (stripos($data,"ga|") !== false)){
$ga = explode("|",$data)[1];
mysqli_query($conn, "UPDATE `azolar` SET `ga`='$ga' WHERE `uid`='$fid'");
$info =  mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `azolar` WHERE `uid`='$fid'"));
$keyb = [];
if($info['dan'] == 'uz'){
    if($info['ga'] == 'ru'){
    ...
    if($info['ga'] == 'uz'){
        array_push($keyb,[["text"=>"🇺🇿 O'zbek","callback_data"=>"dan|uz"],["text"=>"✅ 🇺🇿 O'zbek","callback_data"=>"ga|uz"]]);
        array_push($keyb,[["text"=>"🇷🇺 Russia","callback_data"=>"dan|ru"],["text"=>"🇷🇺 Russia","callback_data"=>"ga|ru"]]);
        array_push($keyb,[["text"=>"✅ 🏴󠁧󠁢󠁥󠁮󠁧󠁿 English","callback_data"=>"dan|en"],["text"=>"❌ English","callback_data"=>"x"]]);
    }else{
        array_push($keyb,[["text"=>"🇺🇿 O'zbek","callback_data"=>"dan|uz"],["text"=>"🇺🇿 O'zbek","callback_data"=>"ga|uz"]]);
        array_push($keyb,[["text"=>"🇷🇺 Russia","callback_data"=>"dan|ru"],["text"=>"✅ 🇷🇺 Russia","callback_data"=>"ga|ru"]]);
        array_push($keyb,[["text"=>"✅ 🏴󠁧󠁢󠁥󠁮󠁧󠁿 English","callback_data"=>"dan|en"],["text"=>"❌ 🏴󠁧󠁢󠁥󠁮󠁧󠁿 English","callback_data"=>"x"]]);
